...
//Code Here
let evenNumbers = mixedNumbers.filter((index) => {
  if(index%2 === 0){
    return index
  }
})
...
//Code Here
let postTaxPrices = prices.map((index) => {
  return index*1.07
});
...
//Code Here
let totalPopulation= populations.reduce((index, current) => {
...
let orderTotals = orders.map((currentValue) => {
  return final = currentValue.price * (1+currentValue.tax)
})
